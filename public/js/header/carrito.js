...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.addEventListener('click', function(e) {
        ...
        if (target.id === 'aumentar-cantidad-carrito' || target.id === 'disminuir-cantidad-carrito') {
            const id = parseInt(target.value);
...
function mostrarCarrito(carritoProductos) {
    ...
    if (carrito.length === 0) {
        carritoProductos.innerHTML = `
            <div class="flex flex-col items-center justify-center py-12 text-gray-400">
                ...
                <a href="/productos" class="mt-4 text-primary hover:underline">Continuar comprando</a>
            </div>
        `;
        return;
    }
    ...
    carrito.forEach((producto) => {
        ...
        productoDiv.className = "bg-custom-dark3 rounded-xl overflow-hidden shadow-lg transition-all duration-300 hover:shadow-xl hover:scale-[1.02] mb-4 last:mb-0";
        ...
        productoDiv.innerHTML = `
            <div class="flex flex-col sm:flex-row gap-6 p-4" id="productoDiv-${producto.id}">
                ...
                <div class="sm:w-1/2 flex flex-col justify-between flex-grow">
                    ...
                    <p class="text-primary font-bold text-xl">${(producto.precio_producto * (producto.cantidad || 1)).toFixed(2)}€</p>
                </div>
                <div class="sm:w-1/4 flex flex-col justify-between gap-4 flex-shrink-0">
                    ...
                    <div class="flex items-center justify-center gap-3 bg-custom-dark2/50 p-2 rounded-lg">
                        <button class="w-8 h-8 flex items-center justify-center text-primary hover:bg-primary/10 rounded-lg transition-all" 
                                id="disminuir-cantidad-carrito" value="${producto.id}" 
                                ${producto.cantidad <= 1 ? 'disabled' : ''}>-</button>
                        <span class="text-lg font-bold text-white">${producto.cantidad || 1}</span>
                        <button class="w-8 h-8 flex items-center justify-center text-primary hover:bg-primary/10 rounded-lg transition-all" 
                                id="aumentar-cantidad-carrito" value="${producto.id}">+</button>
                    </div>
                </div>
            </div>
        `;
...
function actualizarContadores() {
    ...
    if (totalProductos) {
        totalProductos.innerHTML = carrito.reduce((total, producto) => total + (producto.cantidad || 1), 0);
    }
    
    if (totalPrecio) {
        const total = carrito.reduce((sum, producto) => sum + (producto.precio_producto * (producto.cantidad || 1)), 0);
